...
//V1.3
//$uri='/data/v1.2/charbasestat_-(#?)0034355(##)Calypso(?#)_-61_-50_-70_-50_-70_-30_-30.JPG';
//$uri='/data/v1.2/charresiststat_-(#?)0034353(##)CalypsoT(?#)_-61_-0_-0_-0_-44_-0.JPG';
//$uri='/data/v1.2/charbattlestatatk_-(#?)0034353(##)CalypsoT(?#)_-61_-28_-23_-100_-83_-20_-0.JPG';
//$uri='/data/v1.2/charbattlestatdef_-(#?)0034353(##)CalypsoT(?#)_-61_-28_-0_-0_-0_-0.JPG';
$uri=$_SERVER['REQUEST_URI'];
//disreguard ui first load
if($uri=='/data/v1.2/extended_-_-_-.JPG'){
	die;
};
if($uri=='/data/v1.2/char_-_-_-_-.JPG'){
	die;
};
...
if($blow_string[0]=='/data/v1.2/charexp'){
...
if($blow_string[0]=='/data/v1.2/charbasestat'){
	if($account_info[char_list]==""){
		//cannot find character, die.
		die;
	};
	...
	//$blow_string[6]=basicStat.DEX
	//$blow_string[7]=basicStat.INTE
	//$blow_string[8]=basicStat.CHA
	$char_job=@check_job_name($blow_string[1]);
	$char_lvl=$blow_string[2];
	$basicstat_str=$blow_string[3];
	$basicstat_agi=$blow_string[4];
	$basicstat_con=$blow_string[5];
	$basicstat_dex=$blow_string[6];
	$basicstat_int=$blow_string[7];
	$blow_spl8=explode(".JPG", $blow_string[8]);
	$basicstat_cha=$blow_spl8[0];
	...
	for($i=0,$size=count($char_list_blow);$i<$size;$i++){
		...
		if($char_job==$character_data[char_job] && $char_lvl==$character_data[char_lvl]){
			//character found! Update data..
			@$client_mod->update_character_information_basestats_v13($character_data[char_id],$basicstat_str,$basicstat_agi,$basicstat_con,$basicstat_dex,$basicstat_int,$basicstat_cha);
			//we found the character, no longer a need to continue this loop.
			$found=1;
			break;
		}else{
	...
	if($found==0){
		...
		$sql_insert_char_base_data=mysql_query("INSERT INTO clientmod_cache SET account_id='" . $account_id . "', text_info='Character Base Stat Data Received', date='" . $date . "', time='" . $time . "', charbasestat='" . $char_job . ";" . $char_lvl . ";" . $basicstat_str . ";" . $basicstat_agi . ";" . $basicstat_con . ";" . $basicstat_dex . ";" . $basicstat_int . ";" . $basicstat_cha . "'");
	};
	register_shutdown_function('siggen_single',$character_data[char_id],$account_id);
	register_shutdown_function('siggen',$account_id);
};
if($blow_string[0]=='/data/v1.2/charbattlestatatk'){
	if($account_info[char_list]==""){
		//cannot find character, die.
		die;
	};
	...
	//$blow_string[6]=battleStat.CSPD
	//$blow_string[7]=battleStat.CRT
	//$blow_string[8]=battlesStat.DefIP
	$char_job=@check_job_name($blow_string[1]);
	$char_lvl=$blow_string[2];
	$battlestat_ar=$blow_string[3];
	$battlestat_catk=$blow_string[4];
	$battlestat_hr=$blow_string[5];
	$battlestat_cspd=$blow_string[6];
	$battlestat_crt=$blow_string[7];
	$blow_spl8=explode(".JPG", $blow_string[8]);
	$battlestat_defip=$blow_spl8[0];
	...
	for($i=0,$size=count($char_list_blow);$i<$size;$i++){
		...
		if($char_job==$character_data[char_job] && $char_lvl==$character_data[char_lvl]){
			//character found! Update data..
			@$client_mod->update_character_information_battlestats_v13($character_data[char_id],$battlestat_ar,$battlestat_catk,$battlestat_hr,$battlestat_cspd,$battlestat_crt,$battlestat_defip);
			//we found the character, no longer a need to continue this loop.
			$found=1;
			break;
		}else{
	...
	if($found==0){
		//send to clientmod_cache
		$sql_insert_char_atk_data=mysql_query("INSERT INTO clientmod_cache SET account_id='" . $account_id . "', text_info='Character Attack Stat Data Received', date='" . $date . "', time='" . $time . "', charbattlestatatk='" . $char_job . ";" . $char_lvl . ";" . $battlestat_ar . ";" . $battlestat_catk . ";" . $battlestat_hr . ";" . $battlestat_cspd . ";" . $battlestat_crt . ";" . $battlestat_defip . "'");
	};
	register_shutdown_function('siggen_single',$character_data[char_id],$account_id);
	register_shutdown_function('siggen',$account_id);
};
if($blow_string[0]=='/data/v1.2/charbattlestatdef'){
	if($account_info[char_list]==""){
		//cannot find character, die.
		die;
	};
	...
	//$blow_string[6]=battleStat.BLK
	//$blow_string[7]=battleStat.IMP
	$char_job=@check_job_name($blow_string[1]);
	$char_lvl=$blow_string[2];
	$battlestat_dr=$blow_string[3];
	$battlestat_def=$blow_string[4];
	$battlestat_pr=$blow_string[5];
	$battlestat_blk=$blow_string[6];
	$blow_spl7=explode(".JPG", $blow_string[7]);
	$battlestat_imp=$blow_spl7[0];
	...
	for($i=0,$size=count($char_list_blow);$i<$size;$i++){
		...
		if($char_job==$character_data[char_job] && $char_lvl==$character_data[char_lvl]){
			//character found! Update data..
			@$client_mod->update_character_information_battlestatsdef_v13($character_data[char_id],$battlestat_dr,$battlestat_def,$battlestat_pr,$battlestat_blk,$battlestat_imp);
			//we found the character, no longer a need to continue this loop.
			$found=1;
			break;
		}else{
	...
	if($found==0){
		//send to clientmod_cache
		$sql_insert_char_def_data=mysql_query("INSERT INTO clientmod_cache SET account_id='" . $account_id . "', text_info='Character Defense Stat Data Received', date='" . $date . "', time='" . $time . "', charbattlestatdef='" . $char_job . ";" . $char_lvl . ";" . $battlestat_dr . ";" . $battlestat_def . ";" . $battlestat_pr . ";" . $battlestat_blk . ";" . $battlestat_imp . "'");
	};
	register_shutdown_function('siggen_single',$character_data[char_id],$account_id);
	register_shutdown_function('siggen',$account_id);
};
if($blow_string[0]=='/data/v1.2/charresiststat'){
	if($account_info[char_list]==""){
		//cannot find character, die.
		die;
	};
	...
	//$blow_string[6]=resistanceStat.rPsy
	//$blow_string[7]=resistanceStat.rState
	$char_job=@check_job_name($blow_string[1]);
	$char_lvl=$blow_string[2];
	$resistancestat_fire=$blow_string[3];
	$resistancestat_ice=$blow_string[4];
	$resistancestat_light=$blow_string[5];
	$resistancestat_psy=$blow_string[6];
	$blow_spl7=explode(".JPG", $blow_string[7]);
	$resistancestat_state=$blow_spl7[0];
	...
	for($i=0,$size=count($char_list_blow);$i<$size;$i++){
		...
		if($char_job==$character_data[char_job] && $char_lvl==$character_data[char_lvl]){
			//character found! Update data..
			@$client_mod->update_character_information_resistancestat1($character_data[char_id],$resistancestat_fire,$resistancestat_ice,$resistancestat_light,$resistancestat_psy,$resistancestat_state);
			//we found the character, no longer a need to continue this loop.
			$found=1;
			break;
		}else{
	...
	if($found==0){
		//send to clientmod_cache
		$sql_insert_char_res_data=mysql_query("INSERT INTO clientmod_cache SET account_id='" . $account_id . "', text_info='Character Resistance Stat Data Received', date='" . $date . "', time='" . $time . "', charresiststat='" . $char_job . ";" . $char_lvl . ";" . $resistancestat_fire . ";" . $resistancestat_ice . ";" . $resistancestat_light . ";" . $resistancestat_psy . ";" . $resistancestat_state . "'");
	};
	register_shutdown_function('siggen_single',$character_data[char_id],$account_id);
	register_shutdown_function('siggen',$account_id);
};
if($blow_string[0]=='/data/v1.2/extended'){
	...
	$sql_update_family_info=mysql_query("INSERT INTO clientmod_cache SET account_id='" . $account_id . "', text_info='Account Data Received', date='" . $date . "', time='" . $time . "', accountinfo='" . $fam_lvl . ";" . $fam_clan . ";" . $fam_exp . "'");
	register_shutdown_function('siggen',$account_id);
};

if($blow_string[0]=='/data/v1.2/char'){
	//Check for blank strings.
	...
	if($blow_string[3]==""){
		...
		$char_pro='3';
	}else if($blow_string[3]=="High%20Master" && $blow_string[4]>=130){
		$char_pro='4';
	}else if($blow_string[3]=="Grand%20Master" && $blow_string[4]>=140){
		$char_pro='5';
	}else{
		$char_pro='0';
	};
