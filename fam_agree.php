...
if($_POST['url']!=""){
		echo "<div id='content'>";
			...
			if($account_info['family_name']!=""){
				echo "You already have a Family Name set, please contact an administrator if you wish to reset it.";
			...
			if($str_lenght>25){
				echo "Input Family Name is over 25 characters long.";
			...
			if($match_lenght===$str_lenght){
				//Check the string against ALL family names!
				$sql_check_url=mysql_query("SELECT id FROM account_info WHERE family_name='" . $url_dec . "' && server='" . $account_info['server'] . "'");
				if(!$sql_check_url){
			...
			//update
			if($pass=='1'){
				$sql_update_url=mysql_query("UPDATE account_info SET family_name='" . $url . "' WHERE id='" . $account_id . "'");
				if(!$sql_update_url){
   					die('Invalid query: ' . mysql_error());
				};
...
if($_POST['agreecheck']!=""){
		echo "<div id='content'>";
			echo "<div id='simple'>";
				...
				echo "<form action='fam_agree.php' name='custom_url' id='custom_url' method='POST'>";
					echo "Family Name (Max 25 Characters): <input type='text' size='25' name='url' value=''> ";
...
if($_POST['agreecheck']=="" && $_POST['url']==""){
	$site_info=@$site->get_site_agreement(2);
		echo "<div id='content'>";
			echo "<div id='simple'>";
				echo "<h1>Information</h1>";
			if($account_info['family_name']==""){
